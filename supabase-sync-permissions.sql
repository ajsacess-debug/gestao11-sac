...
-- Histórico permanente de reuniões 1:1. É uma inclusão independente das
-- atividades e não altera nenhum registro já existente.
...
create index if not exists meeting_records_participant_meeting_at_idx
on public.meeting_records (participant_username, meeting_at desc);

grant select, insert on table public.meeting_records to authenticated;
alter table public.meeting_records enable row level security;

-- ADM e Gerência acompanham toda a equipe. Coordenação vê e registra
-- somente o próprio histórico de 1:1; os arquivos e acordos ficam protegidos.
drop policy if exists "meeting_records_select" on public.meeting_records;
create policy "meeting_records_select"
  ...
  exists (
    ...
    where requester.id = auth.uid()
      ...
      and (
        ...
        or requester.username = meeting_records.participant_username
        or meeting_records.created_by = auth.uid()
      )
  )
);

drop policy if exists "meeting_records_insert" on public.meeting_records;
create policy "meeting_records_insert"
